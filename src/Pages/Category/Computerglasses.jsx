...
import { FaHeart, FaShoppingCart, FaEye, FaCheck, FaDesktop } from "react-icons/fa";
...
export const Computerglasses = () => {
  ...
  /* ================= FETCH COMPUTER GLASSES ================= */
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await API.get("/products/products");
        const filtered = res.data.filter(
          (p) => p.category_name === "Computer Glasses"
        );
        setProducts(filtered);
      } catch (error) {
        console.error("Failed to fetch computer glasses:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100">
        ...
        <div className="pt-24 pb-16 px-4 bg-gradient-to-r from-blue-900/10 via-indigo-900/10 to-purple-900/10">
          <div className="max-w-7xl mx-auto text-center">
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-white/80 backdrop-blur-sm rounded-2xl mb-6">
              <FaDesktop className="w-8 h-8 text-indigo-600" />
              <span className="text-2xl font-bold text-gray-900">Computer Glasses</span>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-4">
              Digital Eye Protection
            </h1>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto mb-8">
              Premium computer glasses designed to reduce eye strain, block blue light, and enhance comfort during extended screen time.
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              {['Blue Light Blocking', 'Anti-Glare', 'Anti-Fatigue', 'Prescription Ready'].map((feature, i) => (
          ...
          {filteredProducts.length === 0 ? (
            <div className="text-center py-16">
              <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-gray-100 flex items-center justify-center">
                <FaDesktop className="w-12 h-12 text-gray-400" />
              </div>
                ...
                <div>
                  ...
                  <ul className="space-y-3">
                    <li className="flex items-start gap-3">
                      ...
                      <span className="text-gray-700">Blocks up to 99% of harmful blue light</span>
                    </li>
                    <li className="flex items-start gap-3">
                      <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                        <span className="w-2 h-2 bg-blue-600 rounded-full"></span>
                    ...
                    <li className="flex items-start gap-3">
                      ...
                      <span className="text-gray-700">Anti-glare coating for clear vision</span>
                    </li>
                    <li className="flex items-start gap-3">
                      <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                        <span className="w-2 h-2 bg-blue-600 rounded-full"></span>
                      </div>
                      <span className="text-gray-700">Prescription and non-prescription options available</span>
                    </li>
                  </ul>
                </div>
                <div className="text-center">
                  <FaDesktop className="w-24 h-24 text-indigo-600 mx-auto mb-4" />
                  <p className="text-gray-600">
                    Perfect for programmers, gamers, office workers, and anyone spending long hours in front of screens.
                  </p>
                </div>
              </div>
            </div>
